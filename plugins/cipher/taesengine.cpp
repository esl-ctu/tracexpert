...
void TAESEngine::init(bool *ok) {
    ...
    TConfigParam postInitParams = (m_operation==0) ? TConfigParam("Encryption output config", "", TConfigParam::TType::TDummy, "") :  TConfigParam("Decryption output config", "", TConfigParam::TType::TDummy, "");
    ...
    outPar.addSubParam(TConfigParam("M", "0", TConfigParam::TType::TUShort, "Non-negative value M lesser than 128 or 8 respectively"));
...
void TAESEngine::loadKey(){
    ...
    if(m_keyData.length() != m_keysizeB){
        qCritical("Key buffer does not contain a valid amount of bytes (16 B for AES128, 24 B for AES192, 32 B for AES256)");
        return;
    }
