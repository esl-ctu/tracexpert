...
TRandomDevice::TRandomDevice(QString & name, QString & info): m_name(name), m_info(info), m_returnType(TConfigParam::TType::TUShort), m_returnTypeSize(0), m_preInitParamsValid(true), m_initialized(false) {
    ...
    _setDistribution("uniform_int_distribution", m_returnType, "0", "255");
}
...
void TRandomDevice::_createPreInitParams() {
    ...
    TConfigParam rtypeParam = TConfigParam("Result data type", "bit-by-bit", TConfigParam::TType::TEnum, "The result data type generated by the generator.", false);
...
TConfigParam TRandomDevice::setPreInitParams(TConfigParam params) {
    ...
    if(m_returnTypeSize == 0) {
        ...
        _setDistribution("uniform_int_distribution", TConfigParam::TType::TUShort, "0", "255");
...
TConfigParam TRandomDevice::setPostInitParams(TConfigParam params) {
    ...
    if(!_validatePostInitParamsValues(params)) {
        params.setState(TConfigParam::TState::TError, "There are invalid values in the post-init params for TRandomDevice.");
        return params;
    }
...
bool TRandomDevice::_validatePostInitParamsStructure(TConfigParam & params) {
    ...
    if(m_returnTypeSize == 0) {
        ...
        if(params.getSubParamByName("Minimum value (a)")->getType() != m_returnType) return false;

        params.getSubParamByName("Maximum value (b)", &iok);
        if(!iok) return false;

        if(params.getSubParamByName("Maximum value (b)")->getType() != m_returnType) return false;
...
bool TRandomDevice::_validatePostInitParamsValues(TConfigParam & params) {
    ...
    if(m_returnTypeSize == 0) {
        ...
        double min = params.getSubParamByName("Minimum value (a)")->getValue().toDouble();
        double max = params.getSubParamByName("Maximum value (b)")->getValue().toDouble();
        if(min > max) return false;

    } else if(distrParam == "binomial_distribution") {

        TConfigParam * tParam = params.getSubParamByName("Number of trials (t)");
        bool isTLowerThanZero;

        switch(tParam->getType()) {
            case TConfigParam::TType::TShort:
                isTLowerThanZero = tParam->getValue().toShort() < 0;
                break;
            case TConfigParam::TType::TUShort:
                isTLowerThanZero = false;
                break;
            case TConfigParam::TType::TInt:
                isTLowerThanZero = tParam->getValue().toInt() < 0;
                break;
            case TConfigParam::TType::TUInt:
                isTLowerThanZero = false;
                break;
            case TConfigParam::TType::TLongLong:
                isTLowerThanZero = tParam->getValue().toLongLong() < 0;
                break;
            case TConfigParam::TType::TULongLong:
                isTLowerThanZero = false;
                break;
            default:
                isTLowerThanZero = true;
                break;
        }

        if(isTLowerThanZero) {
            params.getSubParamByName("Number of trials (t)")->setState(
                TConfigParam::TState::TWarning,
                "Invalid parameter value, distribution parameter t has to be equal or greater than 0!"
                );
            return false;
        }

        double p2 = params.getSubParamByName("Probability of trial generating true (p)")->getValue().toDouble();
        if(p2 <= 0 || p2 >= 1) {
            params.getSubParamByName("Probability of trial generating true (p)")->setState(
                TConfigParam::TState::TWarning,
                "Invalid parameter value, distribution parameter p has to be between 0 and 1!"
                );
            return false;
        }
        ...
        double p1 = params.getSubParamByName("Probability of trial generating true (p)")->getValue().toDouble();
        if(p1 <= 0 || p1 >= 1) {
            params.getSubParamByName("Probability of trial generating true (p)")->setState(
                TConfigParam::TState::TWarning,
                "Invalid parameter value, distribution parameter p has to be between 0 and 1!"
        ...
        double p1 = params.getSubParamByName("Mean of the distribution (Mu)")->getValue().toDouble();
        if(p1 <= 0) {
            params.getSubParamByName("Mean of the distribution (Mu)")->setState(
                TConfigParam::TState::TWarning,
                "Invalid parameter value, distribution parameter Mu has to be greater than 0!"
                );
            return false;
        }
...
size_t TRandomDevice::readData(uint8_t * buffer, size_t len) {
    ...
    if(m_returnTypeSize == 0) {
        // bit-by-bit mode
        // assumes uniform distribution and short result type in <0, 255> range
        uint8_t tmpBuffer[2];
